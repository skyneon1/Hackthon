...
with gr.Blocks(title="AI Doctor with Vision and Voice") as iface:
    ...
    with gr.Row():
        ...
        with gr.Column():
            text_output = gr.Textbox(label="Your Question (Transcribed)", interactive=False)
            response_output = gr.Textbox(label="Doctor's Response", interactive=False)
    ...
    submit_btn.click(
        ...
        api_name="analyze"  # Add API endpoint name
    )
